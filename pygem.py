...
def calculate_lom(
    positions: np.array, reference_positions: np.array,
    box: np.array
                 ):
    ...
    number_of_rotations = parser.getint('USER', 'number_of_rotations')
    number_of_opt_it = parser.getint('USER', 'number_of_opt_it')
    min_cutoff = parser.getfloat('USER', 'min_cutoff')
    characteristic_distance = parser.getfloat(
            'USER', 'characteristic_distance')
    standard_deviation = parser.getfloat('USER', 'standard_deviation')
    mesh_size = parser.getint('USER', 'mesh_size')
    # Calculate scores
...
def gem(rs: np.array, **kwargs):
    return lambda rs: calculate_lom(
            rs, np.asarray(kwargs.get('reference_positions')),
            np.asarray(kwargs.get('box')))
...
if __name__ == '__main__':
    ...
    box = loadtxt(
            parser['USER']['file_with_sim_box'])
    reference_positions = loadtxt(
            parser['USER']['file_with_reference'])
    positions = loadtxt(
            parser['USER']['file_with_atoms'])

    cv = jit(gem(
        np.array(positions),
        reference_positions=reference_positions,
        box=box))
    mean_score, all_results = cv(np.array(positions))
    # This is only for validation and analysis
    with open('gem_results.pickle', 'wb') as handle:
