...
class PatternCV(CollectiveVariable):
    """
    Collective variable that in addition specifies a reference pattern and
    allows for CV-specific aarguments.
    Parameters
    ----------
    indices: list[int], list[tuple(int)]
        Must be a list or tuple of atoms (ints or ranges) or groups of atoms.
        A group is specified as a nested list or tuple of atoms.
    pattern: JaxArray
        XYZ coordinates of a reference structured used for pattern matching.
    group_length: int, optional
        Specify if a fixed group length is expected.
    kwargs: CV-specific arguments.
    """

    def __init__(
                 self, indices,
                 reference_positions,
                 group_length=None, **kwargs):

        if len(reference_positions) > len(indices):
            raise RuntimeError(
                    "The pattern size is larger than the number of sites")
